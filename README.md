...
channel (`.github/workflows/preview.yml`); a bot comments the preview URL on
the PR and updates it on each push. Previews expire after 7 days. Production
is still served from GitHub Pages.
...
2. Google Cloud console → IAM & Admin → **Service accounts** (project
   `when-can-we-go`) → create one with the **Firebase Hosting Admin** role →
   *Keys* → add a JSON key. Paste the whole JSON as the GitHub repo secret
   `FIREBASE_SERVICE_ACCOUNT_WHEN_CAN_WE_GO` (Settings → Secrets and
   variables → Actions).
